...
@Controller("toto")
public class ProfileController implements Validator {
	...
	/**
	 * Method is used to render the Edit User Profile Form
	 * 
	 * @param model
	 * @return
	 */
	...
	public String update(@ModelAttribute(value = "user") @Validated UserInfo user, BindingResult result, Model model,
		...
		Optional<UserInfo> userUpdated = userOperation.updateUser(user);

		if(!userUpdated.isPresent()) {
			//logger.error("Exception occurred while saving user in Database.", ex);
			redir.addFlashAttribute("errorMessage", "Error while saving user record in database.");
		}

		return "redirect:" + AppConstants.URL_PROFILE;
	}
